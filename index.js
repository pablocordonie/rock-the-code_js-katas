...
console.log(shirtsFilter(products));
...
const placesToTravel_2 = [{ id: 5, name: 'Japan' }, { id: 11, name: 'Venecia' }, { id: 23, name: 'Murcia' }, { id: 40, name: 'Santander' }, { id: 44, name: 'Filipinas' }, { id: 59, name: 'Madagascar' }];

for (let i = 0; i < placesToTravel_2.length; i++) {
  const place = placesToTravel_2[i];
  for (const prop in place) {
    if (place[prop] === 11 || place[prop] === 40) {
      placesToTravel_2.splice(i, 1);
    }
  }
}

console.log(placesToTravel_2);

// Ejercicio 19

// Mixed For e includes: Usa un bucle for para recorrer todos los juguetes y elimina los que incluyan la palabra gato. Puedes usar este array:
...
const toysFilterBySales = (array) => {
  for (const toy of array) {
    if (toy.sellCount > 15) {
      popularToys.push(toy);
      console.log(`${toy.name} ha conseguido más de 15 ventas, con un número total de ${toy.sellCount} ventas`);
    }
  }
  return popularToys;
};

console.log(toysFilterBySales(toys_2));
...
const underAgeUsersFilter = (array) => {
  const underAgeUsers = [];
  array.forEach(user => {
    if (user.age < 18) {
  ...
  return 'Usuarios/as menores de edad: ' + underAgeUsers.map(user => user.name).join(', ');
};

const adultUsersFilter = (array) => {
  const adultUsers = [];
  array.forEach(user => {
    if (user.age >= 18) {
  ...
  return 'Usuarios/as mayores de edad: ' + adultUsers.map(user => user.name).join(', ');
}

console.log(underAgeUsersFilter(users));
console.log(adultUsersFilter(users));
